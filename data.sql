...
create PROC menu_item_edit( @item_id int,@item_name varchar(max),@item_price decimal(10,2),
...
GO
create proc cart_insert (@user_id int,@item_id int) as
begin
insert into cart ([user_id])values(@user_id);
insert into cart (menu_item_id,name,free_delivery,price) select id,Name,Free_Delivery,Price from menu_item where id=@item_id;
end
GO
--5.View Cart (TYUC005)
create proc cart_select (@id int) as 
begin
 select name,free_delivery,price from cart where [user_id]=@id;
 select SUM(price) as 'Total Price' from cart where [user_id]=@id;
 end
 GO  
 --6.Remove Item from Cart (TYUC006)
 create proc cart_delete(@user_id int,@item_id int) as
 begin
